...
namespace System.IO.Compression
{
    ...
    {
        public static void CreateFromDirectories(string sourceDirectoryName, string destinationArchiveFileName, CompressionLevel compressionLevel, bool includeBaseDirectory, Encoding entryNameEncoding, Predicate<string> filter, IProgress<double> progress = null, params string[] targetDirectories)
        {
            if (string.IsNullOrEmpty(sourceDirectoryName)) throw new ArgumentNullException("sourceDirectoryName");
            if (string.IsNullOrEmpty(destinationArchiveFileName)) throw new ArgumentNullException("destinationArchiveFileName");

            //var filesToAdd = Directory.GetFiles(sourceDirectoryName, "*", SearchOption.AllDirectories);
            var filesToAdd = targetDirectories.Select(x=> Directory.GetFiles(sourceDirectoryName+"\\"+x, "*", SearchOption.AllDirectories)).SelectMany(x=>x).Where(x=> filter(x)).ToArray();
            var entryNames = GetEntryNames(filesToAdd, sourceDirectoryName, includeBaseDirectory);
            using (var zipFileStream = new FileStream(destinationArchiveFileName, FileMode.Create))
            {
                using (var archive = new ZipArchive(zipFileStream, ZipArchiveMode.Create))
                {
                    var l = filesToAdd.Length;
                    for (int i = 0; i < l; i++)
                    {
                        archive.CreateEntryFromFile(filesToAdd[i], entryNames[i], compressionLevel);
                        progress?.Report((i + 1.0) / l);
                    }
                }
            }
        }
